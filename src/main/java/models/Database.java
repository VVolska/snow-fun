...
public class Database {
	...
	public void fetchData() throws SQLException {
		...
		// Sklep s = sklepDao.queryForId(1);
		// System.out.println("WINCEJ");
		// List<Sklep> sk = sklepDao.queryForAll();
		// System.out.println("ROZMIAR: " + sk.size());
		// Sklep sklep = sklepDao.queryForId(1);
		// System.out.println("Wybrany sklep " + sklep.getNazwa());
		// List<Sprzet> sp = sprzetDao.queryForAll();

		// Collection<Sprzet> sp = sklep.getSprzety();
		// System.out.println("ILOsc sprzetow w adrenalina: " + sp.size());
		// for(Sprzet i :sp){
		// System.out.println(i.getNazwa());
		// }
		// Sprzet s4 = new Sprzet("LE-KI", Typ.Kijki, 160, 10, true, true);
		//
		// Sklep sklepResult = sklepDao.queryForId(1);
		//
		// Sklep test = new Sklep("Adrenalina", "Kamieniecka 3/5");
		// s4.setSklep(test);

		// sklepDao.create(test);
		// sprzetDao.create(s4);

		// System.out.println(s4.getSklep());
		// System.out.println(sprzetDao.queryForId(5).getNazwa() +
		// sprzetDao.queryForId(5).getSklep());
		// System.out.println("Sklep : "+ sklepResult.getNazwa());
		// ForeignCollection<Sprzet> sprzety = sklepResult.getSprzety();
		Sprzet sprzet = sprzetDao.queryForId(5);
		List<Sprzet> sprzety = sprzetDao.queryForAll();
		for (Sprzet s : sprzety) {
			// if(s.getSklep().equals(sklepResult))
		...
		//
		// Sprzet sprzet =(Sprzet) s.getSprzety().toArray()[1];
		// System.out.println(s.getSprzety().size());
		// System.out.println("Spret: " + sprzet.getNazwa());
	}

	public void createData() throws SQLException {
		ArrayList<Sprzet> sprzety = new ArrayList<>();
		// ArrayList<Sprzet> majestyY = new ArrayList<>();
		...
		// System.out.println(s2.getSklep().getAdres());
		// Sprzet test = sprzetDao.queryForId(2);
		// System.out.println(test.getSklep().getAdres());

		// adrenalinaSklep.setSprzety(sprzety);

		// System.out.println("SZYBKI TESCIK" +
		// adrenalinaSklep.getSprzety().size());
